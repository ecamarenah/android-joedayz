...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE "+sugerenciaTable+" ("+colSugerenciaID+ " INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colSugerenciaEmail+ " TEXT, "+colSugerenciaDesc+" TEXT)");
		
		db.execSQL("CREATE TABLE "+distritoTable+" ("+colDistritoID+ " INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colDistritoName+ " TEXT)");
		
		db.execSQL("CREATE TABLE "+userTable+" ("+colID+ " INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colName+ " TEXT, "+colApePat+" TEXT, "+colApeMat+" TEXT, "+
				colCelular+" INTEGER, "+colEmail+" TEXT, "+colUsuario+" TEXT, "+colClave+" TEXT)");
		
		db.execSQL("CREATE TABLE "+taxiTable+" ("+colTaxiID+ " INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colTaxiName+ " TEXT, "+colTaxiTelefono+" TEXT)");
		
		db.execSQL("CREATE TABLE "+reservaTable+" ("+colReservaID+" INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colReservaUserID+" TEXT, "+colDistritoOrigen+" INTEGER NOT NULL, "+
				colDireccionOrigen+" TEXT, "+colDistritoDestino+" INTEGER NOT NULL, "+
				colDireccionDestino+" TEXT, "+colEmailAlterno+" TEXT, "+
				colObservaciones+" TEXT, "+colFechaReserva+" TEXT, "+
				colHoraReserva+" TEXT, "+colEstadoReserva+" INTEGER NOT NULL, "+
				colReservaTaxiID+" INTEGER, FOREIGN KEY ("+colDistritoOrigen+") REFERENCES "+distritoTable+
				" ("+colDistritoID+"), FOREIGN KEY ("+colDistritoDestino+") REFERENCES "+distritoTable+
				" ("+colDistritoID+"), FOREIGN KEY ("+colReservaUserID+") REFERENCES "+userTable+
				" ("+colUsuario+"));");
		
		/*db.execSQL("CREATE TABLE "+userTable+" ("+colID+" INTEGER PRIMARY KEY AUTOINCREMENT, "+
				colName+" TEXT, "+colAge+" Integer, "+colDistrito+" INTEGER NOT NULL ,FOREIGN KEY ("+colDistrito+") REFERENCES "+distritoTable+" ("+colDistritoID+"));");
		
		
		db.execSQL("CREATE TRIGGER fk_empdistrito_distritoid " +
				" BEFORE INSERT "+
				" ON "+userTable+
				
				" FOR EACH ROW BEGIN"+
				" SELECT CASE WHEN ((SELECT "+colDistritoID+" FROM "+distritoTable+" WHERE "+colDistritoID+"=new."+colDistrito+" ) IS NULL)"+
				" THEN RAISE (ABORT,'Foreign Key Violation') END;"+
				"  END;");
		
		db.execSQL("CREATE VIEW "+viewEmps+
				" AS SELECT "+userTable+"."+colID+" AS _id,"+
				" "+userTable+"."+colName+","+
				" "+userTable+"."+colAge+","+
				" "+distritoTable+"."+colDistritoName+""+
				" FROM "+userTable+" JOIN "+distritoTable+
				" ON "+userTable+"."+colDistrito+" ="+distritoTable+"."+colDistritoID
				);*/
	...
	void AddSugerencia(Sugerencia sug)
	{	 		 
		 SQLiteDatabase db= this.getWritableDatabase();
		 
		 ContentValues cv=new ContentValues();
		
		cv.put(colSugerenciaEmail, sug.getEmail());
		cv.put(colSugerenciaDesc, sug.getSugerencia());
		
		db.insert(sugerenciaTable, colSugerenciaEmail, cv);
		db.close();
	...
	{	 		 
	 ...
	 public int GetReservaID()
	 {
		 ...
		 Cursor c=db.rawQuery("SELECT max("+colReservaID+") as _id FROM "+reservaTable,null);
		 c.moveToFirst();
	 ...
	 public String getTelefonoTaxi(int ID)
	 {
		 ...
		 Cursor c=db.rawQuery("SELECT "+colTaxiTelefono+" FROM "+ taxiTable+" WHERE "+colTaxiID+"=?",params);
		 c.moveToFirst();
	 ...
	 int getValidaUsuario(String usuario, String clave)
	 {
		SQLiteDatabase db=this.getWritableDatabase();
		InsertDistritos(db);
		Cursor cur= db.rawQuery("Select * from "+userTable+" where "+colUsuario+"='"+usuario+"' and "+colClave+"='"+clave+"'", null);
		int x= cur.getCount();
		cur.close();
		return x;
	 }
	 ...
	 {
		 SQLiteDatabase db=this.getReadableDatabase();
		 Cursor cur=db.rawQuery("UPDATE "+reservaTable+" SET "+colReservaTaxiID+"="+idTaxi+" WHERE "+colReservaID+"="+idReserva,null);
	 ...
	 {
		 SQLiteDatabase db=this.getWritableDatabase();
		 //Cursor cur= db.rawQuery("Select "+colID+" as _id , "+colName+", "+colAge+" from "+userTable, new String [] {});
		 //Cursor cur= db.rawQuery("SELECT * FROM "+viewEmps,null);
		 Cursor cur= db.rawQuery("Select "+colID+" as _id , "+colName+", "+colApePat+", "+colApeMat+" from "+userTable, new String [] {});
		 return cur;
	 ...
	 {
		 SQLiteDatabase db=this.getReadableDatabase();
		 Cursor cur=db.rawQuery("SELECT "+colDistritoID+" as _id, "+colDistritoName+" from "+distritoTable,new String [] {});
	 ...
	 {
		 SQLiteDatabase db=this.getReadableDatabase();
		 Cursor cur=db.rawQuery("SELECT "+colTaxiID+" as _id, "+colTaxiName+", "+colTaxiTelefono+" from "+taxiTable+" order by "+colTaxiName,new String [] {});
	 ...
	 Cursor getAllTaxistas()
	 {
		 //crearBD();		 
		 SQLiteDatabase db=this.getReadableDatabase();
		 String sSql="SELECT "+colTaxiID+" as _id, "+colTaxiName+" from "+taxiTable;
	 ...
	 public String GetDistrito(int ID)
	 {
		 ...
		 String[] params=new String[]{String.valueOf(ID)};
		 Cursor c=db.rawQuery("SELECT "+colDistritoName+" FROM"+ distritoTable+" WHERE "+colDistritoID+"=?",params);
		 c.moveToFirst();
	 ...
	 public Cursor getEmpByDistrito(String Distrito)
	 {
		 ...
		 Cursor c=db.query(viewEmps, columns, colDistritoName+"=?", new String[]{Distrito}, null, null, null);
		 return c;
	 }
	 ...
	 {
		 SQLiteDatabase db=this.getReadableDatabase();
		 Cursor c=db.query(distritoTable, new String[]{colDistritoID+" as _id",colDistritoName},colDistritoName+"=?", new String[]{Distrito}, null, null, null);
		 //Cursor c=db.rawQuery("SELECT "+colDistritoID+" as _id FROM "+distritoTable+" WHERE "+colDistritoName+"=?", new String []{Distrito});
		 c.moveToFirst();
		 return c.getInt(c.getColumnIndex("_id"));
		 
		 }
	 
	 public int UpdateEmp(User emp)
	 {
		 SQLiteDatabase db=this.getWritableDatabase();
		 ContentValues cv=new ContentValues();
		 cv.put(colName, emp.getName());
	 ...
	 public void DeleteEmp(User emp)
	 {
		 SQLiteDatabase db=this.getWritableDatabase();
	 ...
	 Cursor getReservas(){
		 ...
		 */
		 ...
		 SQLiteDatabase db=this.getReadableDatabase();
		// String sSql="SELECT * from "+reservaTable;
		// sSql+=" order by 1 desc ";
